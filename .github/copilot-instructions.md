...
- `src/custom-fixtures/` — Custom fixtures extending base test
...
- Import test from `src/custom-fixtures/MergeFixtures.ts`, never from `@playwright/test` directly
...
3. `getByTestId` (attribute is `data-test-id`) and for fast execution
...
- Do not commit `.env`, credentials, `auth.json`, or auth tokens
